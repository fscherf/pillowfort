...
export class Controller {
  ...
  constructor({ engine }: { engine: Engine }) {
    ...
    this.engine.engineElement.addEventListener(
      "focusin",
    ...
    this.engine.engineElement.addEventListener("keydown", this.handleKeyEvent);
    this.engine.engineElement.addEventListener(
      "mouseover",
    ...
    this.engine.engineElement.addEventListener("keyup", this.handleKeyEvent);
    this.engine.engineElement.addEventListener("wheel", this.handleWheelEvent);

    window.addEventListener("keyup", this.handleKeyEvent);
    window.addEventListener("blur", this.handleWindowBlurEvent);

    // disable the browsers context menu
    this.engine.engineElement.addEventListener("contextmenu", (event) => {
  ...
  private updateHoveredLayer(): void {
    ...
    for (
      let index = this.engine.layersSortedByZIndex.length - 1;
      index >= 0;
      index--
    ) {
      const layer: Layer = this.engine.layersSortedByZIndex[index];
  ...
  private handleMouseDownEvent = (event: MouseEvent): void => {
    const rect = this.engine.engineElement.getBoundingClientRect();
